...
import { dummyData2, salesPersons, ShopData } from '../../components/shop';
...
const CuttingPlan = () => {
    ...
    const filteredData = useMemo(() => {
        ...
        return result;
    }, [searchQuery, dateFilter, dateRange, dummyData2]);

    const handleView = (id: string) => {
        navigate(`/shop/edging/${id}`);
    };
    ...
    const columns = useMemo<ColumnDef<ShopData>[]>(
        () => [
            ...
            {
                id: 'edging',
                accessorFn: (row) => row.edging,
                header: ({ column }) => (
                    <DataGridColumnHeader title="Edging" column={column} />
                ),
                cell: ({ row }) => (
                    <span className="text-sm text-text max-w-[200px]">{row.original.edging}</span>
                ),
                enableSorting: true,
            },
            {
