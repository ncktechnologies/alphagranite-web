...
export const TimeTrackingComponent = ({
  ...
  // ✅ FIXED: Parse server timestamps as UTC
  useEffect(() => {
    if (sessionData?.data) {
      const session = sessionData.data;

      // Start time – try different possible field names
      if (session.start_time) {
        setStartTime(parseUTCDate(session.start_time));
      } else if (session.current_session_start_time) {
        setStartTime(parseUTCDate(session.current_session_start_time));
      } else if (draftStart) {
      ...
      if (session.status === 'ended') {
        if (session.end_time) {
          setEndTime(parseUTCDate(session.end_time));
        } else if (session.last_action_time) {
          setEndTime(parseUTCDate(session.last_action_time));
        }
      } else if (draftEnd) {
        setEndTime(draftEnd);
      }
      ...
      if (session.status === 'paused') {
        if (session.paused_at) {
          setPausedTime(parseUTCDate(session.paused_at));
        } else if (session.current_pause_start_time) {
          setPausedTime(parseUTCDate(session.current_pause_start_time));
        }
      }
    } else if (draftStart) {
      setStartTime(draftStart);
  ...
  // Update elapsed time (now using UTC-correct startTime)
  useEffect(() => {
