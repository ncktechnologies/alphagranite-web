...
import { Search, X, CalendarDays, EllipsisVertical, MessageSquare } from 'lucide-react';
...
import { Switch } from '@/components/ui/switch'; // for on‑hold toggle
import { useToggleFabOnHoldMutation, useCreateFabNoteMutation } from '@/store/api/job';
...
export interface CutList {
    ...
    install_date: string;
    sales_person?: string;
    // New fields for permission features
    on_hold?: boolean;
    status_id?: number;
    fab_notes?: any[];
}
...
interface CutListTableProps {
    ...
    // Permission props (to be provided by parent page)
    canAddNote?: boolean;
    canToggleOnHold?: boolean;
    canExport?: boolean;   // optional, default true
}

export const CutListTable = ({ 
    cutLists, 
    path, 
    isSuperAdmin = false, 
    isLoading, 
    ...
    // State for optimistic updates on on‑hold switches
    ...
    const filteredData = useMemo(() => {
        ...
        if (searchQuery) {
            ...
            result = result.filter((list) =>
                list.job_name?.toLowerCase().includes(q) ||
                list.fab_id?.toLowerCase().includes(q) ||
                list.fab_id_0?.toLowerCase().includes(q) ||
                list.job_no?.toLowerCase().includes(q) ||
                list.fab_type?.toLowerCase().includes(q) ||
                list.sales_person?.toLowerCase().includes(q) ||
                list.cip?.toLowerCase().includes(q)
            );
        }

        // Date filter (install_date)
        if (dateFilter !== 'all') {
            result = result.filter((list) => {
                if (dateFilter === 'unscheduled') return !list.install_date || list.install_date === '';
                if (dateFilter === 'scheduled') return list.install_date && list.install_date !== '';
                ...
                switch (dateFilter) {
                    ...
                    case 'custom':
                        ...
                        return true;
                    default: return list.install_date?.includes(dateFilter);
                }
            });
        }
    ...
    const handleView = (fabId: string) => {
        navigate(`/job/${path}/${fabId}`);
    };

    const handleRowClickInternal = (list: CutList) => {
        if (onRowClick) onRowClick(list.fab_id);
    };
    ...
    const handleNoteSubmit = async (note: string, fabId: string, stage?: string) => {
        try {
            await createFabNote({
                ...
                note,
                stage: 'cutlist', // or use a specific stage for cutlist
            }).unwrap();
    ...
    const handleToggleOnHold = async (fabId: string, currentStatus: boolean) => {
        const newStatus = !currentStatus;
        const fabIdStr = String(fabId);
        setOptimisticUpdates(prev => ({ ...prev, [fabIdStr]: newStatus }));
        setLoadingStates(prev => ({ ...prev, [fabIdStr]: true }));
        try {
            await toggleFabOnHold({ fab_id: parseInt(fabId), on_hold: newStatus }).unwrap();
            // keep optimistic update
        } catch (error) {
            setOptimisticUpdates(prev => {
                const copy = { ...prev };
                delete copy[fabIdStr];
                return copy;
            });
            toast.error('Failed to update on-hold status');
        } finally {
            setLoadingStates(prev => ({ ...prev, [fabIdStr]: false }));
        }
    };

    // Build columns dynamically
    const columns = useMemo<ColumnDef<CutList>[]>(() => {
        const baseCols: ColumnDef<CutList>[] = [
            {
                accessorKey: 'id',
        ...
        // Conditionally add On Hold column if user has permission
        if (canToggleOnHold) {
            baseCols.push({
                id: "on_hold",
                accessorKey: "on_hold",
                ...
                cell: ({ row }) => {
                    ...
                    const isChecked = optimisticUpdates[fabId] !== undefined
                        ? optimisticUpdates[fabId]
                        : (row.original.on_hold ?? false);
                    return (
                        <div className="flex justify-center items-center">
                            <Switch
                                ...
                                checked={isChecked}
                                disabled={isLoading}
                                onCheckedChange={() => handleToggleOnHold(fabId, isChecked)}
        ...
        // Actions column with optional "Add Note" item
        baseCols.push({
            id: 'actions',
            header: '',
            cell: ({ row }) => {
                ...
                return (
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                            <DropdownMenuItem onClick={() => handleView(fabId)}>
                                View details
                            </DropdownMenuItem>
                            {canAddNote && (
                                <DropdownMenuItem onClick={() => handleAddNote(fabId)}>
        ...
        return baseCols;
    }, [canToggleOnHold, canAddNote, optimisticUpdates, loadingStates, handleToggleOnHold, handleView, handleAddNote, path]);
    ...
    return (
        <>
                                        ...
                                        <Popover open={isDatePickerOpen} onOpenChange={setIsDatePickerOpen}>
                                            ...
                                            <PopoverContent className="w-auto p-0" align="start">
                                                ...
                                                <div className="flex items-center justify-end gap-1.5 border-t border-border p-3">
                                                    <Button variant="outline" size="sm" onClick={() => { setTempDateRange(undefined); setDateRange(undefined); }}>
                                                        Reset
                                                    </Button>
                                                    <Button size="sm" onClick={() => { setDateRange(tempDateRange); setIsDatePickerOpen(false); }}>
                                                        Apply
                                                    </Button>
                                                </div>
                                            </PopoverContent>
            ...
            {/* Notes Modal */}
            {notesModalOpen && selectedFabId && (
                <NotesModal
                    isOpen={notesModalOpen}
                    onClose={() => setNotesModalOpen(false)}
                    fabId={selectedFabId}
                    onSubmit={(note, fabId) => handleNoteSubmit(note, fabId)}
                />
            )}
        </>
    );
