// SlabSmithSessionHistory.tsx
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { useGetSlabSmithSessionHistoryQuery } from '@/store/api/job';
...
interface SessionActionItem {
    ...
    note: string | null;
    sqft_completed: string | null;        // SlabSmith uses sqft_completed
    work_percentage_done: number | null;  // If backend provides it
}

interface SlabSmithSession {
    session_id: number;
    fab_id: number;
    user_id: number;
    status: string;
...
export const SlabSmithSessionHistory = ({ fabId }: SessionHistoryProps) => {
    const { data: historyData, isLoading, error } = useGetSlabSmithSessionHistoryQuery(fabId, {
        skip: !fabId,
    });

    useEffect(() => {
        if (error) {
            console.error('SlabSmith SessionHistory API Error:', error);
        }
    }, [error]);
    ...
    // Access the sessions array – adjust based on actual API response structure
    const sessionsData: SlabSmithSession[] = historyData?.data?.sessions || [];

    if (sessionsData.length === 0) {
        return null;
    }

    // Combine all actions (notes) from all sessions, sort by timestamp descending
    ...
    return (
        <Card className="mt-4">
            <CardHeader>
                ...
                <p className="text-sm text-muted-foreground">Track your SlabSmith session activities</p>
            </CardHeader>
            <CardContent>
                        ...
                        >
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <div className="flex flex-wrap gap-4 mt-2 text-sm">
                                    {item.sqft_completed !== null && item.sqft_completed !== 0 && (
