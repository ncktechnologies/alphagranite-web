...
export function AvatarInput({
  ...
  // Reset uploaded URL when defaultImageUrl changes (e.g., profile updated externally)
  useEffect(() => {
    setUploadedImageUrl(null);
  }, [defaultImageUrl]);

  // Compute the display URL: uploaded image takes precedence, else defaultImageUrl
  const displayUrl = uploadedImageUrl || defaultImageUrl || null;
      ...
      for (const index of updatedIndexes) {
        ...
        if (fileData?.file) {
          try {
            ...
            formData.append('file_design', 'profile_image');
            formData.append('stage_name', 'profile');
            ...
            const transformedData = {
              ...
              filename: result.data?.name || result.filename || fileData.file.name,
            };

            // Update the displayed image with the uploaded URL
            setUploadedImageUrl(transformedData.url);
            onUploadComplete?.(transformedData);
          } catch (error) {
            console.error('Failed to upload image:', error);
            onUploadError?.(error);
            // Optionally revert the avatar state
          }
        }
      }
    }
  ...
  return (
    <ImageInput value={avatar} onChange={handleAvatarChange}>
      {({ onImageUpload }) => (
        <div className="size-40 relative cursor-pointer" onClick={onImageUpload}>
          <div className="rounded-full overflow-hidden h-full w-full bg-no-repeat bg-cover bg-center flex items-center justify-center border-2 border-green-500">
            {displayUrl ? (
              <img
                src={displayUrl}
                alt={userName}
              ...
              <div className={cn("flex items-center justify-center h-full w-full bg-green-100 text-green-800 font-semibold text-4xl", isLoading && "opacity-50")}>
          ...
          {isLoading && (
            <div className="absolute inset-0 rounded-full bg-black/30 flex items-center justify-center pointer-events-none">
              <LoaderCircleIcon className="h-10 w-10 text-white animate-spin" />
            </div>
          )}

          {/* Upload overlay icon (camera/pencil) */}
          <div className="flex items-center justify-center cursor-pointer rounded-full right-2 bottom-2 bg-primary p-2 absolute">
            <svg
