...
export function OperatorTaskDetails() {
    ...
    const fabId = jobId ? Number(jobId) : 0;
    ...
    const currentTask = tasksData 
        ? (Array.isArray(tasksData) ? tasksData : (tasksData as any)?.data || [])
            .find((task: any) => task.job_id === fabId || task.fab_id === fabId)
        : null;
    ...
    // Get timer history
    const { data: timerHistory } = useGetTimerHistoryQuery({ job_id: fabId }, { skip: !fabId });
        ...
        <>
                    ...
                    <ToolbarHeading
                        title={
                            <div className="text-2xl font-bold">
                                <span>FAB-{currentTask?.fab_id || fabId}: {currentTask?.job_name || 'N/A'}</span>
                                {' - '}
            ...
            <Dialog open={showUploadDialog} onOpenChange={setShowUploadDialog}>
                <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                    <DialogHeader>
                        <DialogTitle>Upload QA Files</DialogTitle>
                    </DialogHeader>
                    <OperatorMediaUpload
                        jobId={currentTask?.job_id || fabId}
                        onUploadComplete={() => {
