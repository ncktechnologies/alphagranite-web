...
export const jobApi = createApi({
    ...
    endpoints(build) {
        return {
            ...
            getFabsCostOfStone: build.query<{ data: Fab[]; total: number }, FabListParams | void>({
                query: (params) => {
                    const queryParams = params || {};
                    return {
                        url: "/fabs/cost-of-stone",
                        method: "get",
                        params: {
                            skip: queryParams.skip || 0,
                            limit: queryParams.limit || 100,
            ...
            updateFabCostOfStone: build.mutation<Fab, { id: number; data: FabUpdate }>({
                query: ({ id, data }) => ({
                    url: `/fabs/${id}/cost-of-stone`,
                    method: "patch",
                    data
                }),
                invalidatesTags: (_result, _error, { id }) => [{ type: "Fab", id }, "Fab"],
